...
describe('fetch-simple base tests', () => {
    ...
    it('get method - passing parameters', (done) => {
        mockFetchResponse(new Response('test'));
        sfApi.get(testUrl, {a:'123', b:'ggg', c: undefined}, {headers: {'Content-Type': 'application/pdf'}})
        .then((result) => {
            expect(result).toEqual('test');
    ...
    it('blob response handled correctly', (done) => {
        ...
        sfApi.get(testUrl, {}, {responseType:'blob'})
        .then((result) => {
...
describe('testing post', () => {
    it('post formdata', (done) => {
        mockFetchResponse(new Response('test'));
        sfApi.post(testUrl, {user: 'test', password: 'dontDoThis', role: undefined})
        .then((result) => {
            expect(result).toEqual('test');
            expect(global.fetch).toHaveBeenCalledWith(testUrl, {
                ...defaultFetchOptions, 
    ...
    it('send file via postFile method', (done) => {
        ...
        mockFetchResponse(new Response('test'));
        sfApi.postFile(testUrl, new Blob(['test']))
        .then((result) => {
            expect(result).toEqual('test');
            expect(global.fetch).toHaveBeenCalledWith(testUrl, {
                ...defaultFetchOptions, 
